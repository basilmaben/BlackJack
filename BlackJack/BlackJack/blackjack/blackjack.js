var cards = []; // Empty array for depositing all our cards

// Creating a deck of cards
var suits = [ 'Spades', 'Hearts', 'Clubs', 'Diamonds' ];  //since, there are 4 different suits in cards
...
var mydollars = 10000;  // starting value
var endplay = false;
//Now,multiple variables to pick up the elements 
...
var pValue = document.getElementById('pValue'); //span to display player cards
var dValue = document.getElementById('dValue'); // span to display dealer cards
...
document.getElementById('mybet').onchange = function() {   //event listener
	...
	message.innerHTML = 'Bet changed to $' + this.value; //displaying the current bet amt
};
...
for (s in suits) {
	...
	for (n in numb) {
		...
		var card = {                     
			suit: suit,    //suit {Diam,Hearts,Spade,Club}
			icon: suits[s],   // icon
			bgcolor: bgcolor,  // Red or Black
			cardnum: numb[n],  // number of Card {A-K}
			cardvalue: cardValue // knowing the card's worth (A can be 1 or 11)
		};
		cards.push(card); 
	}
}
...
function Start() {
	shuffleDeck(cards);  // Randomizing the order of cards
...
function dealNew() {       // No arguments, because we simply dealing out the cards
  dValue.innerHTML = '?';  //Doesnt display the value of Dealers card until showdown
  playerCard = [];         // clear out the dealer's and player's card
	...
	document.getElementById('mybet').disabled = true;  //unclickable 
	document.getElementById('maxbet').disabled = true; // unclickable
	deal();  //
	document.getElementById('btndeal').style.display = 'none'; // hides the deal key
}
...
function playend() {
	endplay = true;
	document.getElementById('cover').style.display = 'none';  //removes the cover from the dealers card
	document.getElementById('myactions').style.display = 'none';  //cant hit stand or double
	document.getElementById('btndeal').style.display = 'block';  //Deal 
	...
	if (playervalue == 21 && playerCard.length == 2) {       //quick check to see if it's a blackjack
		...
		payoutJack = 1.5;  //multiplies winning amount by 1.5 times
		playend();  //start of new game
	}
...
function checktotal(arr) {
	...
	if (aceAdjust && rValue > 21) {  //auto adjusting the value to 1 or 11 depending upon the value
		rValue = rValue - 10;
	}
	return rValue;
}
